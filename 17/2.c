...
int rc;
int cc;
unsigned int cost[1024][1024][4][10];
unsigned int cest[1024][1024][4][10];
...
int from(state s, state *out) {
  ...
  for (int i = -1; i < 2; i++) {
    ...
    int dc = i == 0 ? s.dc + 1 : 0;
    if (s.r == 0 && s.c == 0) {
      // this is weird but fixes the answer
      dc = i == 0 ? s.dc : 0;
    }
...
int main(void) {
  ...
  memset(cost, -1, 1024 * 1024 * 4 * 4 * 10);
  rc = 0;
  while (1) {
    char buf[1024];
